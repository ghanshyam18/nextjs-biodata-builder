...
const FloralTemplate: React.FC<TemplateProps> = ({ data }) => {
  ...
  const Row = ({ label, value }: { label: string; value: string }) => {
    ...
    return (
      <Flex py={6} align="flex-start" gap={4} style={{ borderBottom: '1px solid var(--mantine-color-pink-1)' }}>
        <Text span w={128} size="xs" fw={600} c="pink.7" tt="uppercase" lts={1} pt={2} style={{ flexShrink: 0 }}>{label}</Text>
        <Text span size="sm" fw={500} c="var(--mantine-color-blue-9)">{value}</Text>
      </Flex>
    );
  };
  ...
  const SectionTitle = ({ title }: { title: string }) => (
    <Flex align="center" gap="sm" my="md">
      <Text span c="pink.3" size="lg" style={{ userSelect: 'none' }}>✿</Text>
      <Title order={3} size="sm" fw={700} c="pink.7" tt="uppercase" lts={2}>{title}</Title>
  ...
  return (
    <Box style={{ minHeight: '100%', position: 'relative', overflow: 'hidden', background: 'linear-gradient(160deg, #fff0f5 0%, #fce7f3 50%, #fdf2f8 100%)' }}>
      <Box pos="absolute" top={12} left={12} c="pink.2" fz={36} style={{ userSelect: 'none', lineHeight: 1 }}>❀</Box>
      <Box pos="absolute" top={12} right={12} c="pink.2" fz={36} style={{ userSelect: 'none', lineHeight: 1 }}>❀</Box>
      <Box pos="absolute" bottom={12} left={12} c="pink.2" fz={36} style={{ userSelect: 'none', lineHeight: 1 }}>❀</Box>
      <Box pos="absolute" bottom={12} right={12} c="pink.2" fz={36} style={{ userSelect: 'none', lineHeight: 1 }}>❀</Box>
      <Box pos="absolute" top={8} left={8} right={8} bottom={8} style={{ border: '2px solid var(--mantine-color-pink-2)', borderRadius: 'var(--mantine-radius-md)', pointerEvents: 'none' }}></Box>

      <Box pos="relative" style={{ zIndex: 10 }} px={24} py={28}>
        <Flex align="flex-start" gap="md" mb="md">
          <Box flex={1} ta="center">
            <Box c="pink.3" fz={24} mb={8} lts={2} style={{ userSelect: 'none' }}>✦ ✦ ✦</Box>
            <Title order={1} size="h3" fw={700} c="pink.7" tt="uppercase" lts={2} mb={4}>Biodata</Title>
            {personalDetails.fullName && (
              <Title order={2} size="h5" fw={600} c="var(--mantine-color-red-8)" mt={4}>{personalDetails.fullName}</Title>
            )}
            <Box c="pink.3" fz={18} mt={8} lts={2} style={{ userSelect: 'none' }}>~ ❀ ~</Box>
          </Box>
          {personalDetails.photo && (
            <Box style={{ flexShrink: 0, border: '2px solid var(--mantine-color-pink-3)', padding: 2, borderRadius: 'var(--mantine-radius-sm)', background: '#fdf2f8' }}>
              <img src={personalDetails.photo} alt="Profile" style={{ width: 64, height: 80, objectFit: 'cover', borderRadius: 'var(--mantine-radius-sm)' }} />
            </Box>
          )}
        </Flex>
        ...
        <Box ta="center" mt={24} c="pink.3" fz={18} lts={2} style={{ userSelect: 'none' }}>✦ ✦ ✦</Box>
      </Box>
    </Box>
  );
};
