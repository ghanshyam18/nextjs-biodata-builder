...
const ElegantTemplate: React.FC<TemplateProps> = ({ data }) => {
  ...
  const Row = ({ label, value }: { label: string; value: string }) => {
    ...
    return (
      <Flex py={6} align="flex-start" gap="sm" style={{ borderBottom: '1px solid #e8dfc8' }}>
        <Text span w={128} fz={12} fw={700} tt="uppercase" lts={1} pt={2} style={{ flexShrink: 0, color: GOLD }}>{label}</Text>
        <Text span fz="sm" fw={500} style={{ color: NAVY }}>{value}</Text>
      </Flex>
    );
  };
  ...
  const SectionTitle = ({ title }: { title: string }) => (
    <Flex align="center" gap="sm" mt={24} mb={12}>
      <Text span fz="xs" style={{ color: GOLD, userSelect: 'none' }}>◆</Text>
      <Title order={3} fz="xs" fw={700} tt="uppercase" style={{ letterSpacing: '0.2em', color: GOLD }}>{title}</Title>
  ...
  return (
    <Box style={{ minHeight: '100%', backgroundColor: 'white', fontFamily: 'serif' }}>
      <Box pos="relative" px={24} py={32} style={{ backgroundColor: NAVY }}>
        <Box mb={12} h={1} w="100%" style={{ background: `linear-gradient(to right, transparent, ${GOLD}, transparent)` }}></Box>
        <Flex align="center" gap="md">
          <Box flex={1} ta="center">
            <Box fz="xs" tt="uppercase" mb={8} style={{ letterSpacing: '0.3em', color: GOLD }}>— Biodata —</Box>
            ...
            {educationCareer.occupation && (
              <Text c="slate.4" fz="sm" mt={4} style={{ fontFamily: 'sans-serif' }}>{educationCareer.occupation}</Text>
            )}
          </Box>
          {personalDetails.photo && (
            <img src={personalDetails.photo} alt="Profile" style={{ width: 64, height: 80, objectFit: 'cover', flexShrink: 0, border: `1.5px solid ${GOLD}` }} />
          )}
        </Flex>
        <Box mt={12} h={1} w="100%" style={{ background: `linear-gradient(to right, transparent, ${GOLD}, transparent)` }}></Box>
      </Box>
      ...
      <Box px={24} pb={32} style={{ backgroundColor: '#fffef9' }}>
        ...
        <Box mt={32} h={1} w="100%" style={{ background: `linear-gradient(to right, transparent, ${GOLD}, transparent)` }}></Box>
      </Box>
    </Box>
  );
};
