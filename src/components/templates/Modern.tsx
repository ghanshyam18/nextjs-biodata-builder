...
const ModernTemplate: React.FC<TemplateProps> = ({ data }) => {
  ...
  const Row = ({ label, value }: { label: string; value: string }) => {
    ...
    return (
      ...
      >
        <Text span size="xs" c="gray.6" fw={500} tt="uppercase" lts={1}>
          {label}
        </Text>
        <Text span c="dark.8" fw={500} size="sm" mt={2}>
          {value}
        </Text>
      </Flex>
    );
  ...
  const SectionTitle = ({ title }: { title: string }) => (
    <Title
      ...
      mt="xl"
      w="100%"
      style={{ borderBottom: '2px solid var(--mantine-color-blue-5)' }}
    >
      {title}
    </Title>
  );
