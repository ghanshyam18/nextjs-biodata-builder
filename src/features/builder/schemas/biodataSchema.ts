...
export const personalDetailsSchema = z.object({
  fullName: z.string().trim().min(1, 'Full Name is required').max(50, 'Max 50 characters allowed').refine(val => !/[<>]/.test(val), 'Invalid characters detected'),
  dateOfBirth: z.string().default(''),
  timeOfBirth: z.string().default(''),
  placeOfBirth: z.string().max(100, 'Max 100 characters allowed').refine(val => !/[<>]/.test(val), 'Invalid characters detected').default(''),
...
export const contactDetailsSchema = z.object({
  mobileNumber: z.string().refine(val => !val || /^[\d\s+\-()]{10,15}$/.test(val), '10-15 digits allowed').default(''),
  email: z.string().refine(val => !val || /^\S+@\S+\.\S+$/.test(val), 'Invalid email address').default(''),
  address: z.string().max(200, 'Max 200 characters allowed').refine(val => !val || !/[<>]/.test(val), 'Invalid characters detected').default(''),
});
